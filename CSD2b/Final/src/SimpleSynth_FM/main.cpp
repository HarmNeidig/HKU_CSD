...
int main(int argc,char **argv)
{
  ...
  int choiceLength;
  int nTimes;
  std::cout << "What waveform do you want to use for the carrier?" << std::endl;
  std::cout << "1 = Sine, 2 = Saw, 3 = Square" << std::endl;
  std::cout << "Your choice: ";
  std::cin >> choiceCar;
  if (choiceCar < 1 || choiceCar > 3){
  ...
  Osc* carrier = synth.selectOsc(choiceCar);
  Osc* modulator = synth.selectOsc(choiceMod);
  modulator->setFrequency(8.8);
  //assign a function to the JackModule::onProces
  jack.onProcess = [&carrier,&modulator,&n,&gen,&nTimes](jack_default_audio_sample_t *inBuf,
